...
const CreateT3Comment: NextPage = () => {
  const { data, isLoading, refetch } = api.comments.getAllT3Comments.useQuery();
  const createT3Comment = api.comments.createT3Comment.useMutation();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)

  const createComment = async (comment: string, author = 'Anonymous') => {
    await createT3Comment.mutateAsync({ comment, author }, {
        onSuccess: () => {
          void refetch();
        },
      onError: (error) => {
  ...
  return (
    <>
      <Head>
        <title>Content Moderation - T3</title>
        <meta name="description" content="Content Moderation T3" />
...
export default CreateT3Comment;
