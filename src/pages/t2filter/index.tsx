...
const CreateT2Comment: NextPage = () => {
  const { data, isLoading, refetch } = api.comments.getAllT2Comments.useQuery();
  const createT2Comment = api.comments.createT2Comment.useMutation();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)

  const createComment = async (comment: string, author = 'Anonymous') => {
    await createT2Comment.mutateAsync({ comment, author }, {
        onSuccess: () => {
          void refetch();
        },
      onError: (error) => {
  ...
  return (
    <>
      <Head>
        <title>Content Moderation - T2</title>
        <meta name="description" content="Content Moderation T2" />
...
export default CreateT2Comment;
